    ...
    (cond
     ...
     [(pair? datum)
      (cond
		...
		[(and (eqv? (car datum) 'let*) (validate-let datum))
		; let*
		(let*-exp
			(map car (cadr datum))
...
(define syntax-expand
  (lambda (exp)
	(cases expression exp
		...
		[let*-exp (syms exprs bodies)
			(syntax-expand (let*->let syms (map syntax-expand exprs) (map syntax-expand bodies)))]
...
(define let*->let
	(lambda (syms exprs bodies)
		(cond [(null? (cdr exprs)) (let-exp (list (car syms)) (list (car exprs)) bodies)]
			[else (let-exp (list (car syms)) (list (car exprs)) (list (let*->let (cdr syms) (cdr exprs) bodies)))])))
...
(define *prim-proc-names* '(+ - * / add1 sub1 set-car! set-cdr! not car cdr caar cadr cadar symbol? list list? list->vector vector->list vector? vector vector-ref number? length pair? cons >= = > < <= zero? null? eq? equal? procedure? map apply quotient vector-set! eqv? list-tail append assq display))
